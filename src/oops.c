...
)
{
  ...
  int numThreads = n_samples;
  if(numThreads > MAX_NUM_THREADS) //check and decide the number of threads
    numThreads = MAX_NUM_THREADS;
  // M step 
  double * q_buf = (double *)malloc(n_samples * sizeof(double));
  double * loglike0_buf = (double *)malloc(n_samples * sizeof(double));
  double ** obs_buf = (double **)malloc(n_samples * sizeof(double *));
  //create_2array(ptr, double, model->max_w+1, alph_size_core(dataset->alph));
  ...
  for (i=0; i<n_samples; i++) 
  {
    ...
    q_buf[i] = 0;
    loglike0_buf[i] = 0;
    obs_buf[i] = (double *)malloc(w * alen_core * sizeof(double));
    memset(obs_buf[i], 0, w * alen_core * sizeof(double));
  }
  ...
  shared(back, samples, n_samples, q_buf, loglike0_buf, w, invcomp, \
    dataset,  alen_core, obs_buf)\
  private(i, j, k, ii, jj) schedule(runtime)
...
//FIXME
//printf("m_step: %s lseq %d group %d skip %d\n", s->sample_name, lseq, s->group, dataset->skip[s->group]);
    ...
    for (k=0, j=1; k<m; k++,j++) {      // k=seq. pos., j: Z_ij = 1 (Z_i = j)
      ...
      for (ii=0,r=s->res+off; ii<w; ii++,r++) { // position 
        if (*r < alen_core) { // normal letter 
          ...
          *(obs_buf[i] + ii * alen_core +*r) += z;
        } else { // 'X': spread z over all 
          for (jj=0; jj < alen_core; jj++) 
            //obs(ii, jj) += z * get_array_item(jj, back);
            *(obs_buf[i] + ii * alen_core + jj) += z * get_array_item(jj, back);
        }
      } // sequence position 
      //loglike0 += z * Log_back(lcb, k, w);      // Pr(sites | \theta0)
      loglike0_buf[i] += z * Log_back(lcb, k, w); 
      qi += z; // sum of z_ij 
//FIXME:
//if (k==0) printf("m_step: %s qi: %g\n", s->sample_name, qi);
  ...
  for (i=0; i<n_samples; i++) 
  {
      loglike0 += loglike0_buf[i];
      q += q_buf[i];
      
      for (ii=0; ii<w; ii++) {
        for (jj=0; jj < alen_core; jj++) 
          obs(ii, jj) += *(obs_buf[i] + ii * alen_core + jj); 
      }
...
)
{
  ...
  if(numThreads > MAX_NUM_THREADS) //check and decide the number of threads
    numThreads = MAX_NUM_THREADS;
  double * logpX_buf= (double *)malloc(n_samples * sizeof(double));
  // E step 
  ...
  #pragma omp parallel  for num_threads(numThreads) default(none)  \
  shared(n_samples, logtheta1, logtheta1_rc, w, invcomp, \
    dataset, mtype,  gamma, log_1mgamma, logpX_buf)\
  private(i, j, k, ii) schedule(runtime)
  for (i=0; i < n_samples; i++){ // sequence 
...
//FIXME:
//if (k==0) printf("s: %s init %g log_pij %g log_pXijtheta1: %g\n", s->sample_name, init, log_Pij[j], log_pXijtheta1);
      // Initialize log Pr(X_i | Z_i=-j, \theta1)
